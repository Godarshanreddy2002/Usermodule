...
export async function ActivateOrDeactivateUser(req: Request, params: Record<string, string>): Promise<Response> {
  try {
    ...
    console.log(LOGINFO.USER_STATUS_UPDATE_STARTED.replace("{user_id}", id).replace("{status}", account_status));  // Log when status update starts
    ...
    if (isvalidAccountStatus instanceof Response) {
      console.error(LOGERROR.INVALID_ACCOUNT_STATUS.replace("{status}", account_status)); 
    ...
    if (idavailble instanceof Response) {
      console.error(LOGERROR.INVALID_USER_ID.replace("{user_id}", id));  
      return idavailble;
    }

    
    if (params.user_type != 'A') {
      console.error(LOGERROR.NOT_ALLOWED_TO_CHANGE_USER_STATUS.replace("{user_id}", id));  
      return ErrorResponse(HTTP_STATUS_CODE.FORBIDDEN, USERMODULE.NOT_ALLOWED);
    }

    console.log(LOGINFO.USER_STATUS_UPDATE_PROCESS.replace("{user_id}", id)); 

    
    const { data, error: updateError } = await updateUserStatus(id, account_status);
    if (updateError) {
      console.error(LOGERROR.INTERNAL_SERVER_ERROR.replace("{error}", updateError));  
      return ErrorResponse(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, `${USERMODULE.INTERNAL_SERVER_ERROR} : ${updateError.message}`);
    }

    if (!data) {
      console.error(LOGERROR.USER_NOT_FOUND.replace("{user_id}", id));  
      return ErrorResponse(HTTP_STATUS_CODE.NOT_FOUND, USERMODULE.USER_NOT_FOUND_);
    }

    console.log(LOGINFO.USER_STATUS_UPDATED.replace("{user_id}", id).replace("{status}", account_status));  
